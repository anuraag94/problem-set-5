...
###figure out auc
glimpse(gss_logit_cv10)

gss_logit_cv10 %>%
  roc_curve(truth = colrac, .estimate) %>%
  autoplot()
...
boost_train_mses <- map_dbl(lambda, boosted_tree_train)

boosted_mses <- as.data.frame(cbind(lambda, boost_train_mses)) %>%
  rename(mse = `boost_train_mses`)

ggplot() +
  geom_point(data = boosted_mses, mapping = aes(lambda, mse), color = "blue") +
  geom_line(data = boosted_mses, mapping = aes(lambda, mse), color = "red") +
